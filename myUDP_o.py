...
class Testkao():
    ...
    def _getEmpiricalData(self): 
        ...
        self.empiFC = empiri['FC_Env_mean'].astype(np.float32) # empiprical functional connectivity
        self.empiProfile = []
    ...
    def _doNodeContainers(self,maxDlay):      
        ...
        phi[:,0:maxDlay+1]  = np.tile(np.remainder(omegaT,2*np.pi, dtype=np.float32),(self.C,1))
        return r, phi    
